...
import {
  FollowerUser,
  T_FollowerUserConstructorParametersJSON,
} from "@/types/classes/model/Follow/Follower/Follower";
...
const fetchRequests = async (): Promise<Array<FollowerUser>> => {
  try {
    const response = await axiosCredentialInstance.get<
      Response<T_FollowerUserConstructorParametersJSON[]>
    >(`/follow/follower/0`);

    switch (response.status) {
      case OK_HTTP_RESPONSE_CODE:
        return response.data.data.map(FollowerUser.createFromJSON);
