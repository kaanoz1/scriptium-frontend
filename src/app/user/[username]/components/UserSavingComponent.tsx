...
import {
  Collection,
  T_CollectionConstructorParametersJSON,
...
import CollectionUpdateModal from "@/components/collection/CollectionUpdateModal";
  ...
  const fetchCollections = async () => {
    try {
      const response = await axiosCredentialInstance.get<
        Response<Array<T_CollectionConstructorParametersJSON>>
      >(`/collection/`);

      switch (response.status) {
        case OK_HTTP_RESPONSE_CODE:
          setError(undefined);
          return response.data.data.map(Collection.createFromJSON);
