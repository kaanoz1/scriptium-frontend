...
import GetUserRoleSymbolsComponent from "@/app/user/[username]/components/GetUserRoleSymbolsComponent";
import PrivateAccountIcon from "@/app/user/[username]/components/PrivateAccountIcon";
...
import UserProfilePrivateProfile from "@/app/user/[username]/components/UserProfilePrivateProfile";
import UserProfileTabs from "@/app/user/[username]/components/UserProfileTabs";
import UserPageRejectFollowerConfirmationModal from "@/app/user/[username]/components/UserRejectFollowerConfirmationModal";
...
import {
  UserFetched,
  T_UserFetchedConstructorParametersJSON,
...
const Page: NextPage = () => {
  ...
  const isOwnProfile = user; //&& user.getId() === userFetched.getId(); // Own profile check is disabled since user context is not available.

  const formattedName = userFetched.getFormattedName();
  ...
  const imagePath = userFetched.getImage();
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
            <div className="flex-shrink-0 flex justify-center sm:justify-start w-full sm:w-auto">
              {imagePath ? (
                ...
                <Avatar className="h-32 w-32" size="lg" name={formattedName} />
              )}
            </div>
...
const fetchUserByUsername = async (
  username: string
): Promise<UserFetched | T_AuthenticationRequestErrorCode> => {
  try {
    const response = await axiosCredentialInstance.get<
      Response<T_UserFetchedConstructorParametersJSON>
    >(`/user/${username}`);

    if (response.status === OK_HTTP_RESPONSE_CODE)
      return UserFetched.createFromJSON(response.data.data);
...
const setUserFetched = (
  ...
  queryClient.setQueryData<UserFetched | null>(queryKey, (prev) => {
    let updated: UserFetched | null;
