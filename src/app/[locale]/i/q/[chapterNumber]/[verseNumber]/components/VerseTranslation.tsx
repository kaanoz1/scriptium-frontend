...
const VerseTranslation: React.FC<Props> = observer(({ verse }) => {
  const preferredTranslations =
    QuranTranslationPreferences.getInstance().preferredTranslations;
  ...
  for (const t of preferredTranslations) {
    const translationFound = verse.translations.find(
      (translation) => translation.translation.id === t.id,
    );

    if (translationFound) translationShown.push(translationFound.translation);
  }
  ...
  return (
    <main className="flex flex-col gap-6 w-full">
      {translationShown.map((transl, index) => {
        ...
        const authorNamesConcatenated = transl.authors
          .map((author) => author.name)
          .filter((name) => name != translationName)
          .join(", ");

        const translationText =
          verse.translations.find(
            (translation) => translation.translation.id === transl.id,
          )?.text ?? t("VerseBoxTranslation.NoProvidedTranslation");
        ...
        return (
          ...
          >
            ...
            <p
              className={`text-base leading-relaxed text-foreground/90 ${!isSingleTranslation ? "pl-3 border-l-2 border-primary/20" : ""}`}
            >
              {translationText}
            </p>
          </div>
