...
const QuranTranslationChangeButton: React.FC = observer(() => {
  ...
  const t = useTranslations(
    "Pages.Islam.Quran.Chapter.Components.VerseContainer.VerseContainerHeader.UtilToolButtons.QuranTranslationChangeButton",
  );
      ...
      <DialogContent className="sm:max-w-md max-h-[80vh] flex flex-col">
        ...
        <main className="grid gap-3 py-4 overflow-y-auto pr-2">
          {allTranslations.map((translation) => {
            const authorNames = translation.authors
              .map((a) => a.name)
              .filter((name) => name != translation.name)
              .join(", ");
            ...
            return (
              <div
                ...
                onClick={() => {
                  const currentLength =
                    translationPreferences.preferredTranslations.length;
