...
type Props = {
  book: Readonly<BookCover>;
  partNode: BookNodeCover;
  childBookSubPartNode: BookNodeCover;
};

const BookSubPartBox: FC<Props> = ({
  book,
  partNode,
  childBookSubPartNode,
}) => {
  const bookMeaning = book.getMeaningTextOrDefault(DEFAULT_LANG_CODE);

  const childBookNodeMeaning =
    childBookSubPartNode.getMeaningTextOrDefault(DEFAULT_LANG_CODE);

  const childBookNodeName = childBookSubPartNode.getName();

  const bookNodeMeaning = partNode.getMeaningTextOrDefault(DEFAULT_LANG_CODE);

  const description = partNode.getDescription() ?? "Click to read.";
  return (
    <motion.div
      variants={{
    ...
    >
      <Link
        color="foreground"
        href={`/books/${bookMeaning}/${bookNodeMeaning}/${childBookNodeMeaning}`}
      ...
      >
        <h2 className="font-semibold mb-1 dark:text-white">
          {childBookNodeMeaning} ({childBookNodeName})
        </h2>
