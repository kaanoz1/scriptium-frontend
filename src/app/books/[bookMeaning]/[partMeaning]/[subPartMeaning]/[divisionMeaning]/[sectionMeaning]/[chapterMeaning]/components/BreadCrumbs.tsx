...
const BreadCrumbs: FC<T_BreadCrumbsParams> = ({ chapterNode }) => {
  // Chapter
  const chapterName = chapterNode.getName();
  const chapterMeaning = chapterNode.getMeaningTextOrDefault(DEFAULT_LANG_CODE);
  ...
  const chapterUrl = `${sectionUrl}/${chapterMeaning ?? chapterName}`;
  ...
  return (
    <Breadcrumbs size="lg">
      ...
      <BreadcrumbItem href={chapterUrl}>
        {chapterMeaning}{" "}
