...
type Props = {
  ...
  chapterNode: Readonly<BookNodeCover>;
  subChapterNode: Readonly<BookNodeCover>;
};

const BookSubChapterBox: FC<Props> = ({
  book,
  partNode,
  subPartNode,
  divisionNode,
  ...
  const chapterNodeMeaning =
    chapterNode.getMeaningTextOrDefault(DEFAULT_LANG_CODE);

  const chapterNodeName = chapterNode.getName();

  //Book -> Part -> SubPart -> Division -> Section -> Chapter -> SubChapter

  const subChapterNodeMeaning =
    subChapterNode.getMeaningTextOrDefault(DEFAULT_LANG_CODE);
  const subChapterNodeName = subChapterNode.getName();

  const description = subChapterNode.getDescription() ?? "Click to read.";

  const heading = subChapterNodeMeaning
    ? `${subChapterNodeMeaning} (${subChapterNodeName})`
    : subChapterNodeName;
  ...
  return (
    ...
    >
      <Link
        ...
        }/${chapterNodeMeaning ?? chapterNodeName}/${
          subChapterNodeMeaning ?? subChapterNodeName
        }`}
...
export default BookSubChapterBox;
