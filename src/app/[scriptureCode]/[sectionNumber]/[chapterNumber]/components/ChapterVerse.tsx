...
const ChapterVerse: FC<Props> = ({
    ...
    const transliteration: string | ReactNode = verse
        .getTransliterationTextOrNull(DEFAULT_LANG_CODE) ?? (
        <span className="italic">No transliteration available.</span>
    );

    const verseNumber: number = verse.getNumber();
    const verseText: string =
        verse.getTextOfVariationOrUsual(preference.getPreferredOriginalScriptureTextVariationKey())
    ...
    const selectedTranslations: Array<TranslationDTO> = scriptureDetail.getTranslations().filter(t => preference.getPreferredTranslationIdMultiple().has(t.getId()))
    const preferredTranslationTexts = verse.getTranslationTexts().filter(tt => selectedTranslations.map(t => t.getId()).includes(tt.getTranslation().getId()))
                ...
                >
                    <GrShareOption
                        ...
                        onClick={() => {
                            4
                            functionWhichOpensShareModel();
                            setStateActionFunctionForShareText(getShareTextOfVerse(verse, chapter, preferredTranslationTexts)
                            );
                        }}
                    />
                ...
                <div className="text-base font-light leading-relaxed">
                    <div className="flex-column gap-2">
                        {selectedTranslations.map(tt => {
                            const correspondingTranslationText = verse.getTranslationTexts().find(ttx => ttx.getTranslation().getId() == tt.getId())

                            if (correspondingTranslationText)
                                return <TranslatedTextWithFootnotes
