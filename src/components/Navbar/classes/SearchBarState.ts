...
export class SearchBarState {
  ...
  private _searchAlgorithm: SearchAlgorithm =
  ...
  public static getInstance(): SearchBarState {
    if (this._instance === null) this._instance = new SearchBarState();
  ...
  async search() {
    ...
    const quickNavigation = QuickNavigationResult.produce(trimmed);
    if (quickNavigation) {
    ...
    try {
      if (this.algorithm instanceof TranslationSearchAlgorithm) {
        ...
        if (
          this.algorithm.methodology === TranslationSearchMethodology.TextSearch
        ) {
          const result = await controller.textSearch(this._query);
          this._result = result.data.verses.map(
            VerseTranslationWithVerse.fromJson,
          );
          return;
        }
        if (
          this.algorithm.methodology ===
      ...
      this._result = new ErrorResult(
        "Error",
        "Failed to search. This might be a temporary issue. Please try again later.",
      );
    } finally {
      this._isLoading = false;
    }
