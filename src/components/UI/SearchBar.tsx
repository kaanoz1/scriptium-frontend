...
import QueryResultSectionBar from "@/components/main/QueryResultSectionBar";
import QueryResultVerseBar from "@/components/main/QueryResultVerseBar";
...
const parseQueryToRoute = (query: string): string | null => {
  ...
  if (new ScriptureHelperCollection().isCodeValid(codeCandidate ?? "t")) {
        ...
        >
          {(queryData?.getSearchResultArray() ?? []).map((row) => {
            ...
            if (row instanceof TranslationTextWithVerseUpperMean) {
              ...
              return (
                <AutocompleteItem
                  classNames={{ base: "w-90vw sm:w-auto" }}
