...
> = {
    t: ScriptureDetail.create({
        ...
        code: "t",
        variation: new TextVariationSymbols({usual: "אָ֑", simplified: "א", withoutVowel: "א"}),
...
export const UNKNOWN_LANGUAGE: LanguageDTO = new LanguageDTO(
    {langCode: "en", langEnglish: "unknown", langOwn: "unknown"}
);
...
export const UNDEFINED_TRANSLATION_TEXT_DTO: TranslationTextDTO =
    new TranslationTextDTO({
        text: "Unknown Text", translation: UNKNOWN_TRANSLATION, footNotes: [
            UNKNOWN_FOOTNOTES,
        ]
    });
...
export const getArrangedVerseAndTranslationFor = (
    ...
    for (const verse of verses) {
        const scripture: Readonly<ScriptureDTO> = verse.getChapter().getSection().getScripture();
        ...
        const translation: TranslationTextDTO =
            verse.getTranslationTexts().find((tt) => tt.getTranslation().getId() == preferredTranslationId) ??
            verse.getTranslationTexts().find((tt) => tt.getTranslation().getId() == defaultTranslationId)!;

        arr.push(new VerseTranslationTextPair({verse, translationText: translation}));
    }
...
export const getShareTextForVersePage = (
    ...
    return `${scriptureMeaning} (${scriptureNameInOwnLanguage}), ${sectionMeaning} (${sectionNameInOwnLanguage}), Chapter: ${chapterNumber}, Verse: ${verseNumber}\n
                      ${verseText}\n\n${translationTexts.filter((t) => preferredTranslationIds.has(t.getTranslation().getId())).map((e) => `${e.getTranslation().getName()}\n ${e.getText()}`).join("\n")}\n\n${window.location.href}`;
};
...
export const isNoAuthenticationRequestErrorCode
    : (input: T_NoAuthenticationRequestErrorCode | unknown) => input is T_NoAuthenticationRequestErrorCode = (input: T_NoAuthenticationRequestErrorCode | unknown) => {
    return input === UNAUTHORIZED_HTTP_RESPONSE_CODE || input === NOT_FOUND_HTTP_RESPONSE_CODE || input === TOO_MANY_REQUEST_HTTP_RESPONSE_CODE || input === INTERNAL_SERVER_ERROR_HTTP_RESPONSE_CODE || input === CONFLICT_HTTP_RESPONSE_CODE;
}

export const isAuthenticationRequestErrorCode
    : (input: T_AuthenticationRequestErrorCode | unknown) => input is T_AuthenticationRequestErrorCode = (input: T_AuthenticationRequestErrorCode | unknown) => {
    return isNoAuthenticationRequestErrorCode(input) || input == METHOD_NOT_ALLOWED_HTTP_RESPONSE_CODE;
}
