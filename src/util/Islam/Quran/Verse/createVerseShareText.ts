...
export const createVerseShareText = (
  chapter: ChapterComplete,
  verse: VerseComplete,
  translations: Array<TranslationPlain>,
  locale: string,
) => {
  const chapterNumber = chapter.sequence;
  const chapterMeaning =
    chapter.meanings.find((meaning) => meaning.language.code === locale)
      ?.text ??
    chapter.meanings.find((meaning) => meaning.language.code === "en")?.text ??
    chapter.name;
  ...
  for (const t of translations) {
    const foundVerseTranslation = verse.translations.find(
      (vt) => vt.translation.id === t.id,
    );
  ...
  if (typeof window !== "undefined")
    result += `scriptium.net/${locale}/i/q/${chapterNumber}/${verseNumber}`;
